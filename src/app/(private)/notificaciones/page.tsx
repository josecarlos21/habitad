...
import { Bell, Package, Users, Wrench } from "lucide-react";
            ...
            {isLoading ? (
                ...
                <div className="space-y-4">
                    {notifications.map((notification, i) => {
                        const category = categoryMap[notification.category];
                        return (
                            <Card 
                                key={notification.id}
                            ...
                            >
                                <CardContent className="flex items-center gap-4 p-4">
                                    ...
                                    <div className={cn("grid h-10 w-10 place-items-center rounded-lg shrink-0", !notification.read ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground")}>
                                        <category.icon className="h-5 w-5" />
                                    </div>
                                    <div className="flex-1">
                                        <p className="font-semibold text-sm">{notification.title || category.defaultTitle}</p>
